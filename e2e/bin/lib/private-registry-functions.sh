...
use_private_registry() {
  yarn_config_set() {
    scopeRegistryConfig='{
      "npmAlwaysAuth":true,
      "npmAuthToken":"anonymous",
      "npmRegistryServer":"${NPM_PUBLISH_REGISTRY:-http://localhost:4873/}",
      "npmPublishRegistry":"${NPM_PUBLISH_REGISTRY:-http://localhost:4873/}"
    }'
    yarn config set 'npmScopes["team-monite"]' --json "$scopeRegistryConfig" || return 1
    yarn config set 'npmScopes["monite"]' --json "$scopeRegistryConfig" || return 1
    yarn config set 'unsafeHttpWhitelist' --json '["${UNSAFE_HTTP_WHITELIST:-localhost}"]' || return 1
  }

  echo 'Patching `.yarnrc.yml` file to use private registry...'
  yarn_config_set >> /dev/null || (echo 'Error: Failed to patch `.yarnrc.yml` file.' >&2 && exit 1)
}

# Remove patch from `.yarnrc.yml` file
use_default_registry() {
  yarn_config_set() {
    yarn config unset 'npmScopes["team-monite"]' || return 1
    yarn config unset 'npmScopes["monite"]' || return 1
    yarn config unset 'unsafeHttpWhitelist' || return 1
  }

  echo 'Removing patch from `.yarnrc.yml` file...'
  yarn_config_set >> /dev/null || (echo 'Error: Failed to remove patch from `.yarnrc.yml` file.' >&2 && exit 1)
}
