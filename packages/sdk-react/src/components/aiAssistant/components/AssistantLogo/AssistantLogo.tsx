import React from 'react';

import { SvgIcon } from '@mui/material';
import { SvgIconOwnProps } from '@mui/material/SvgIcon/SvgIcon';

export const AssistantLogo = (props: SvgIconOwnProps) => (
  <SvgIcon sx={{ fontSize: 51 }} {...props}>
    <svg
      width="51"
      height="51"
      viewBox="0 0 51 51"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <svg
        width="51"
        height="51"
        viewBox="0 0 51 51"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <mask
          id="mask0_65_2657"
          style={{ maskType: 'alpha' }}
          maskUnits="userSpaceOnUse"
          x="0"
          y="0"
          width="51"
          height="51"
        >
          <circle
            cx="25.2302"
            cy="25.2302"
            r="25.2302"
            fill="url(#paint0_linear_65_2657)"
          />
        </mask>
        <g mask="url(#mask0_65_2657)">
          <circle
            cx="25.2302"
            cy="25.2302"
            r="25.2302"
            fill="url(#paint1_linear_65_2657)"
          />
          <foreignObject x="-29.267" y="2.01841" width="91.838" height="91.838">
            <div
              style={{
                backdropFilter: 'blur(5.05px)',
                clipPath: 'url(#bgblur_0_65_2657_clip_path)',
                height: '100%',
                width: '100%',
              }}
            ></div>
          </foreignObject>
          <g filter="url(#filter0_f_65_2657)">
            <circle
              cx="16.652"
              cy="47.9374"
              r="20.6888"
              fill="#FFAEC7"
              fillOpacity="0.7"
            />
          </g>
          <foreignObject
            x="-2.01849"
            y="-36.8361"
            width="91.838"
            height="91.838"
          >
            <div
              style={{
                backdropFilter: 'blur(5.05px)',
                clipPath: 'url(#bgblur_1_65_2657_clip_path)',
                height: '100%',
                width: '100%',
              }}
            ></div>
          </foreignObject>
          <g filter="url(#filter1_f_65_2657)">
            <circle cx="43.9005" cy="9.08288" r="20.6888" fill="white" />
          </g>
        </g>
        <path
          d="M32.1359 32.7523C46.0216 23.22 40.4951 10.0945 29.4302 5.9833C24.2697 4.0659 25.058 11.2162 18.6456 17.5455C12.2332 23.8748 4.02084 33.9538 12.5884 38.3315C21.1559 42.7092 26.2708 36.7786 32.1359 32.7523Z"
          fill="url(#paint2_radial_65_2657)"
          fillOpacity="0.1"
        />
        <path
          d="M32.1359 32.7523C46.0216 23.22 40.4951 10.0945 29.4302 5.9833C24.2697 4.0659 25.058 11.2162 18.6456 17.5455C12.2332 23.8748 4.02084 33.9538 12.5884 38.3315C21.1559 42.7092 26.2708 36.7786 32.1359 32.7523Z"
          fill="url(#paint3_linear_65_2657)"
          fillOpacity="0.1"
        />
        <path
          d="M32.1359 32.7523C46.0216 23.22 40.4951 10.0945 29.4302 5.9833C24.2697 4.0659 25.058 11.2162 18.6456 17.5455C12.2332 23.8748 4.02084 33.9538 12.5884 38.3315C21.1559 42.7092 26.2708 36.7786 32.1359 32.7523Z"
          fill="url(#paint4_radial_65_2657)"
          fillOpacity="0.1"
        />
        <path
          d="M11.0531 23.3823C13.2814 16.2813 13.4011 5.95839 21.5299 5.95839C27.9473 5.95839 33.1899 8.66202 36.5039 14.0905C39.8179 19.5191 46.2165 20.6276 46.2165 24.638C46.2165 31.9142 33.3867 35.7217 25.5706 34.961C17.7545 34.2003 8.82494 30.4833 11.0531 23.3823Z"
          fill="url(#paint5_linear_65_2657)"
          fill-opacity="0.1"
          style={{ mixBlendMode: 'plus-lighter' }}
        />
        <path
          d="M11.0531 23.3823C13.2814 16.2813 13.4011 5.95839 21.5299 5.95839C27.9473 5.95839 33.1899 8.66202 36.5039 14.0905C39.8179 19.5191 46.2165 20.6276 46.2165 24.638C46.2165 31.9142 33.3867 35.7217 25.5706 34.961C17.7545 34.2003 8.82494 30.4833 11.0531 23.3823Z"
          fill="url(#paint6_radial_65_2657)"
          fill-opacity="0.1"
        />
        <path
          d="M27.1625 38.7261C28.9397 46.5128 41.4747 43.5634 36.0675 36.6815C30.6602 29.7995 30.8966 30.2613 26.5445 25.1296C26.2166 30.117 25.3854 30.9393 27.1625 38.7261Z"
          fill="url(#paint7_radial_65_2657)"
          fill-opacity="0.1"
        />
        <path
          d="M27.1625 38.7261C28.9397 46.5128 41.4747 43.5634 36.0675 36.6815C30.6602 29.7995 30.8966 30.2613 26.5445 25.1296C26.2166 30.117 25.3854 30.9393 27.1625 38.7261Z"
          fill="url(#paint8_linear_65_2657)"
          fill-opacity="0.1"
        />
        <g clip-path="url(#paint9_angular_65_2657_clip_path)">
          <g transform="matrix(0.0116194 -0.0249308 0.0262203 0.0122204 26.3079 26.8501)">
            <foreignObject
              x="-963.326"
              y="-963.326"
              width="1926.65"
              height="1926.65"
            >
              <div
                style={{
                  background:
                    'conic-gradient(from 90deg,rgba(250, 168, 170, 0.9687) 0deg,rgba(255, 173, 173, 1) 2.76959deg,rgba(255, 50, 5, 0) 92.9253deg,rgba(255, 240, 224, 0) 205.268deg,rgba(122, 13, 78, 0) 274.405deg,rgba(250, 168, 170, 0.9687) 360deg)',
                  height: '100%',
                  width: '100%',
                  opacity: 1,
                }}
              ></div>
            </foreignObject>
          </g>
        </g>
        <path
          d="M8.36563 21.4428C2.52161 8.41046 28.52 -2.39828 24.2831 8.5599C20.0462 19.5181 30.8008 37.3067 38.4188 23.7941C44.929 12.2467 14.2097 34.4752 8.36563 21.4428Z"
          style={{
            mixBlendMode: 'screen',
          }}
        />
        <path
          d="M25.7131 32.6935C19.6717 32.0116 17.4597 30.7826 18.3396 25.32C19.2195 19.8573 22.1689 21.0863 25.7131 17.9465C29.2573 14.8066 31.3268 17.5657 34.4188 23.4091C37.5107 29.2525 31.7545 33.3755 25.7131 32.6935Z"
          fill="url(#paint10_radial_65_2657)"
          fill-opacity="0.3"
        />
        <defs>
          <filter
            id="filter0_f_65_2657"
            x="-29.267"
            y="2.01841"
            width="91.838"
            height="91.838"
            filterUnits="userSpaceOnUse"
            color-interpolation-filters="sRGB"
          >
            <feFlood flood-opacity="0" result="BackgroundImageFix" />
            <feBlend
              mode="normal"
              in="SourceGraphic"
              in2="BackgroundImageFix"
              result="shape"
            />
            <feGaussianBlur
              stdDeviation="12.6151"
              result="effect1_foregroundBlur_65_2657"
            />
          </filter>
          <clipPath
            id="bgblur_0_65_2657_clip_path"
            transform="translate(29.267 -2.01841)"
          >
            <circle cx="16.652" cy="47.9374" r="20.6888" />
          </clipPath>
          <filter
            id="filter1_f_65_2657"
            x="-2.01849"
            y="-36.8361"
            width="91.838"
            height="91.838"
            filterUnits="userSpaceOnUse"
            color-interpolation-filters="sRGB"
          >
            <feFlood flood-opacity="0" result="BackgroundImageFix" />
            <feBlend
              mode="normal"
              in="SourceGraphic"
              in2="BackgroundImageFix"
              result="shape"
            />
            <feGaussianBlur
              stdDeviation="12.6151"
              result="effect1_foregroundBlur_65_2657"
            />
          </filter>
          <clipPath
            id="bgblur_1_65_2657_clip_path"
            transform="translate(2.01849 36.8361)"
          >
            <circle cx="43.9005" cy="9.08288" r="20.6888" />
          </clipPath>
          <clipPath id="paint9_angular_65_2657_clip_path">
            <path
              d="M8.36563 21.4428C2.52161 8.41046 28.52 -2.39828 24.2831 8.5599C20.0462 19.5181 30.8008 37.3067 38.4188 23.7941C44.929 12.2467 14.2097 34.4752 8.36563 21.4428Z"
              style={{ mixBlendMode: 'screen' }}
            />
          </clipPath>
          <linearGradient
            id="paint0_linear_65_2657"
            x1="25.2302"
            y1="0"
            x2="25.2302"
            y2="50.4604"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="#8A78DF" />
            <stop offset="1" stop-color="#ECF6FF" />
          </linearGradient>
          <linearGradient
            id="paint1_linear_65_2657"
            x1="25.2302"
            y1="0"
            x2="25.2302"
            y2="50.4604"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="#8A78DF" />
            <stop offset="1" stop-color="#ECF6FF" />
          </linearGradient>
          <radialGradient
            id="paint2_radial_65_2657"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(11.1426 23.2558) rotate(32.5356) scale(20.8514 21.3209)"
          >
            <stop stopColor="#C1506B" />
            <stop offset="1" stopColor="#F7BADE" stopOpacity="0" />
          </radialGradient>
          <linearGradient
            id="paint3_linear_65_2657"
            x1="15.7616"
            y1="12.6155"
            x2="19.8735"
            y2="17.8199"
            gradientUnits="userSpaceOnUse"
          >
            <stop stopColor="#C150A2" />
            <stop offset="1" stopColor="#E0ADBC" stopOpacity="0" />
          </linearGradient>
          <radialGradient
            id="paint4_radial_65_2657"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(27.7137 25.3916) rotate(109.542) scale(12.423 11.2071)"
          >
            <stop stopColor="#FF2E00" />
            <stop offset="1" stopColor="#FFD6CD" stopOpacity="0" />
          </radialGradient>
          <linearGradient
            id="paint5_linear_65_2657"
            x1="40.4042"
            y1="29.0196"
            x2="45.2616"
            y2="19.7274"
            gradientUnits="userSpaceOnUse"
          >
            <stop stopColor="#EE518C" stopOpacity="0" />
            <stop offset="0.665732" stopColor="#EE518C" />
            <stop offset="0.911525" stopColor="#FF7DAE" />
          </linearGradient>
          <radialGradient
            id="paint6_radial_65_2657"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(12.2982 24.3922) rotate(30.6997) scale(9.14702 11.1627)"
          >
            <stop stopColor="#8F4791" />
            <stop offset="1" stopColor="#8F4791" stopOpacity="0" />
          </radialGradient>
          <radialGradient
            id="paint7_radial_65_2657"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(33.1899 43.3176) rotate(-102.011) scale(11.8105 15.1254)"
          >
            <stop stopColor="#E375DF" stopOpacity="0" />
            <stop offset="1" stopColor="#FF91B2" />
          </radialGradient>
          <linearGradient
            id="paint8_linear_65_2657"
            x1="27.1837"
            y1="27.2928"
            x2="27.9237"
            y2="32.3081"
            gradientUnits="userSpaceOnUse"
          >
            <stop stopColor="white" />
            <stop offset="1" stopColor="white" stopOpacity="0" />
          </linearGradient>
          <radialGradient
            id="paint10_radial_65_2657"
            cx="0"
            cy="0"
            r="1"
            gradientUnits="userSpaceOnUse"
            gradientTransform="translate(25.7131 25.32) rotate(-180) scale(7.37354 7.37353)"
          >
            <stop stopColor="white" />
            <stop offset="0.165839" stopColor="white" />
            <stop offset="0.335281" stopColor="#FFD18B" />
            <stop offset="1" stopColor="#FF144D" stopOpacity="0" />
          </radialGradient>
        </defs>
      </svg>
    </svg>
  </SvgIcon>
);
