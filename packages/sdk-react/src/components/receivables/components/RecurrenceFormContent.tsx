...
export const RecurrenceFormContent = ({ isUpdate = false }) => {
  const { control, watch, setValue } =
    useFormContext<CreateRecurrenceFormProps>();
  const { i18n } = useLingui();
  const recurrenceStartDateValue = watch('recurrence_start_date');
  const recurrenceIssueMode = watch('recurrence_issue_mode');
  const recurrenceStartDate = recurrenceStartDateValue
    ? new Date(recurrenceStartDateValue)
    : undefined;
              ...
              <FormControl>
                <DatePicker
                  ...
                  startMonth={
                    recurrenceStartDate
                      ? new Date(
                          recurrenceStartDate.getFullYear(),
                          recurrenceStartDate.getMonth() + 2,
                          0
                        )
                      : new Date()
                  }
        ...
        render={({ field }) => (
          <FormItem className="mtw:flex-1">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="first_day">{t(
                  i18n
                )`Start of the month`}</SelectItem>
                <SelectItem value="last_day">{t(
                  i18n
                )`End of the month`}</SelectItem>
              </SelectContent>
            </Select>
            <FormMessage />
          </FormItem>
