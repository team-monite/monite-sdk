...
export const MarkAsUncollectibleModal = ({
  ...
  const { mutate: markAsUncollectible, isPending: isMarkingAsUncollectible } = useMarkAsUncollectibleReceivableById(
    invoiceId,
    onMarkAsUncollectible
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
        <DialogContent showCloseButton={false}>
            <DialogHeader>
                <DialogTitle>
                    {t(i18n)`Mark invoice as uncollectible?`}
                </DialogTitle>
                <DialogDescription>
                    <p>
                        {t(i18n)`This invoice will change its status, and you won't be able to receive or log any payment against it.`}
                    </p><br />
                    <p>
                        {t(i18n)`For bookkeeping purposes, this is a write-off and has tax implications.`}
                    </p>
                </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
                ...
                <Button onClick={handleMarkAsUncollectible} disabled={isMarkingAsUncollectible}>
                    {isMarkingAsUncollectible && <LoadingSpinner className="mtw:w-5 mtw:h-5 mtw:border-inherit mtw:border-t-transparent" />}
