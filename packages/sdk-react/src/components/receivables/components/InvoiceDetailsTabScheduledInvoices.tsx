...
const IterationInvoice = ({ iteration, invoice, i18n, onRowClick }: IterationInvoiceProps) => {
    const { locale } = useMoniteContext();
    const { formatCurrencyToDisplay } = useCurrencies();
    const { data: iterationInvoice } = useGetReceivableById(iteration?.issued_invoice_id ?? '', Boolean(iteration?.issued_invoice_id));
    ...
    return (
        ...
        >
            <td className="mtw:text-left mtw:h-9 mtw:py-2 mtw:pr-2 mtw:text-sm mtw:leading-5 mtw:font-normal">
                {iteration?.issued_invoice_id ? iterationInvoice?.document_id : INVOICE_DOCUMENT_AUTO_ID}
            </td>
...
export const InvoiceDetailsTabScheduledInvoices = ({ 
    ...
    return (
        <table>
            <thead>
                <tr>
                    <th className="mtw:text-left mtw:h-10 mtw:pr-2 mtw:text-sm mtw:font-medium mtw:leading-5">{t(i18n)`Invoice number`}</th>
                    <th className="mtw:text-left mtw:h-10 mtw:px-2 mtw:text-sm mtw:font-medium mtw:leading-5">{t(i18n)`Status`}</th>
                    <th className="mtw:text-left mtw:h-10 mtw:px-2 mtw:text-sm mtw:font-medium mtw:leading-5">{t(i18n)`Date`}</th>
                    <th className="mtw:text-right mtw:h-10 mtw:pl-2 mtw:text-sm mtw:font-medium mtw:leading-5">{t(i18n)`Amount`}</th>
                </tr>
            </thead>
            <tbody>
