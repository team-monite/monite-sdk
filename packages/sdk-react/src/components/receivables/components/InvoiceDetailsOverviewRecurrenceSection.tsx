import {
  getCreateRecurrenceValidationSchema,
  CreateRecurrenceFormProps,
} from '../InvoiceDetails/CreateReceivable/validation';
...
export const InvoiceDetailsOverviewRecurrenceSection = ({
  ...
  const issuedDocuments = useMemo(
    () =>
      recurrence?.iterations?.filter(
        (iteration) => iteration.status === 'completed'
      ),
    [recurrence]
  );
  const { mutateAsync: updateRecurrence, isPending: isUpdatingRecurrence } =
    useUpdateRecurrenceById(recurrence?.id, recurrence?.invoice_id);
            ...
            >
              <SheetContent className="mtw:gap-0" showCloseButton={false}>
                <SheetHeader className="mtw:border-b mtw:border-border">
                  <SheetTitle>{t(i18n)`Edit recurrence settings`}</SheetTitle>
                  <SheetDescription hidden>{t(
                    i18n
                  )`Edit the settings for this recurring invoice`}</SheetDescription>
                </SheetHeader>
      ...
      <div className="mtw:flex mtw:flex-col mtw:gap-4">
        ...
        <div className="mtw:grid mtw:grid-cols-2 mtw:gap-4">
          <InvoiceDetailsInfoBlock
            ...
            value={
              recurrence?.start_date
                ? i18n.date(recurrence?.start_date, {
                    month: 'long',
                    year: 'numeric',
                  })
                : '-'
          ...
          <InvoiceDetailsInfoBlock
            ...
            value={t(
              i18n
            )`Every ${startDay === 1 ? 'first' : 'last'} day of month`}
          />
          <InvoiceDetailsInfoBlock
            label={t(i18n)`Issued documents`}
            value={t(
              i18n
            )`${issuedDocuments.length} / ${recurrence?.iterations.length} issued`}
            onClick={() => {
        ...
        {isCreatedFromRecurrence ? (
          <Button
            ...
            size="sm"
            className="mtw:w-fit"
            onClick={() => openInvoiceDetails?.(recurrence?.invoice_id)}
          >
