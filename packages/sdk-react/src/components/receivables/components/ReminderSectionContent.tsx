...
export const ReminderSectionContent = ({
  ...
  const counterpartDefaultContactEmail = counterpartContacts?.find(
    (contact) => contact?.is_default
  )?.email;

  const validReminderEmail = counterpartDefaultContactEmail
  ...
  const hasValidReminderEmailLoading =
    isCounterpartLoading || isCounterpartContactsLoading;
    ...
    <>
      ...
      {validReminderEmail && (
        <p className="mtw:text-muted-foreground mtw:text-sm mtw:font-medium">
          {i18n._(`Recipient email: ${validReminderEmail}`)}
          <br />
