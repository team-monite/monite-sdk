...
export const getEntityBankAccountValidationSchema = (
  ...
  const baseSchema = z.object({
    country: z
      .enum([...AllowedCountries, ''] as const)
      .meta({ title: t(i18n)`Country` }),

    currency: z
      .enum([...CurrencyEnum, ''] as const)
    ...
    iban: z
      ...
      .meta({
        ...
        description: t(
          i18n
        )`International Bank Account Number (required for EUR)`,
      }),
    ...
    sort_code: z
      ...
      .meta({
        ...
        description: t(
          i18n
        )`6-digit code required for non-EUR and non-USD currencies`,
      }),
  });
  ...
  const schemaWithValidation = baseSchema
    ...
    .refine(
      (data) => {
        if (data.currency === 'USD' || data.currency === 'GBP') {
          return (
            data.account_holder_name && data.account_holder_name.length > 0
          );
        }
        return true;
      },
    ...
    .refine(
      ...
      },
      {
        message: t(
          i18n
        )`Sort code is required and must be exactly 6 characters`,
        path: ['sort_code'],
      }
    )
    .refine(
...
export const getEntityProfileValidationSchema = (i18n: I18n) => {
  return z.object({
    title: z
      .string()
      .optional()
      .meta({ title: t(i18n)`Title` }),
    name: z
      .string()
      .optional()
      .meta({ title: t(i18n)`Name` }),
    surname: z
      .string()
      .optional()
      .meta({ title: t(i18n)`Surname` }),
    email: z
