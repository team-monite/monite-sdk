...
export const getCurrencyEnum = (_i18n: I18n) => CurrencyEnumSchema;
...
const getCreateBankAccountSchema = (i18n: I18n) =>
  entityBankAccountSchema
    .extend({
      country: z
        .string()
        .min(1, t(i18n)`Country is required`)
        .meta({ title: t(i18n)`Country` }),
      currency: getCurrencyEnum(i18n).meta({ title: t(i18n)`Currency` }),
      is_default_for_currency: z
        .boolean()
